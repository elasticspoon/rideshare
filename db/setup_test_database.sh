...
# ROLES
echo "SELECT 'CREATE USER $APP_TEST_USER WITH LOGIN PASSWORD ''postgres''' WHERE NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$APP_TEST_USER')\gexec" | psql $DB_URL
...
psql -U $APP_TEST_USER -h localhost -d $APP_TEST_DB_NAME -c "CREATE SCHEMA rideshare;"
psql -U $APP_TEST_USER -h localhost -d $APP_TEST_DB_NAME -c "ALTER ROLE $APP_TEST_USER SET search_path TO rideshare;"
psql -U $APP_TEST_USER -h localhost -d $APP_TEST_DB_NAME -c "SET search_path TO rideshare;"
